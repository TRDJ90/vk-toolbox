...
const Instance = vktb.instance.Instance;
const PhysicalDevice = vktb.physicaldevice.PhysicalDevice;
const PhysicalDeviceSelector = vktb.physicaldevice.PhysicalDeviceSelector;
const PhysicalDeviceSelectorConfig = vktb.physicaldevice.PhysicalDeviceSelectorConfig;
const Device = vktb.device.Device;
...
pub fn main() !void {
    ...
    try vktb.instance.getInstanceSurfaceExtensions(&instance_extensions);

    // TODO: should be in instance init.
    ...
    defer p_device_selector.deinit(allocator);
    ...
    const device: Device = try Device.init(
        ...
        pdev.handle,
        surface,
        @ptrCast(device_extensions.items),
        &features,
    );

    defer device.deinit(allocator);

    vktb.vma.init(vktb.vma.VMAConfig{
        .instance = instance.handle,
        .physical_device = pdev.handle,
        .device = device.handle,
        .getInstanceProcAddr = instance.base.dispatch.vkGetInstanceProcAddr,
        .getDeviceProcAddr = instance.wrapper.dispatch.vkGetDeviceProcAddr,
    });
